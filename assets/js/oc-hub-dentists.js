/**
 * CoverCapy — OC Hub Dentist Grid
 * Fetches OC dentists from Supabase, sorts Platinum Elite first,
 * then by google_review_count. Paginates at 9/page.
 *
 * Required elements:
 *   <div id="cc-oc-grid"></div>
 *   <p  id="cc-oc-note"></p>   (optional)
 */
(function () {
  'use strict';
  ...
  function renderPage(container, noteEl, elite, rest, page) {
    ...
    // Separator between elite and rest within a page
    let cards = '';
    ...
    slice.forEach((d, i) => {
      ...
      if (!crossedBoundary && globalIdx === elite.length) {
        cards += `<div class="cc-oc-section-break">
          <hr class="cc-oc-hr">
          <p class="cc-oc-section-label" style="margin-top:20px">All PPO offices in Orange County</p>
        </div>`;
  ...
  <button class="cc-oc-pg-btn" ${page >= totalPages - 1 ? 'disabled' : ''} data-page="${page + 1}">Next →</button>
    ...
    if (noteEl) {
      noteEl.textContent = elite.length > 0
        ? `${elite.length} Platinum Elite office${elite.length !== 1 ? 's' : ''} · ${all.length} total offices in CoverCapy's OC directory · page ${page + 1} of ${totalPages}`
        : `${all.length} offices in CoverCapy's Orange County directory`;
    }
  }
...
`;
  ...
  async function init() {
    const grid = document.getElementById('cc-oc-grid');
    if (!grid) return;

    const noteEl = document.getElementById('cc-oc-note');
    injectCSS();
    grid.innerHTML = `<div class="cc-oc-loading">Loading Orange County dental offices…</div>`;

    try {
      const rows = await fetchAll();

      // Sort: platinum_elite first, then by google_review_count desc
      ...
      console.warn('CoverCapy oc-hub-dentists:', err);
